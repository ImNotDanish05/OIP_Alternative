...
func _ready():
	...
	# For capsule-only player (OIP: no Skeleton3D), sync default heights to actual CapsuleShape so crouch works with 1.7 height scene values
	if skeleton_ref == null and collision_shape_ref and collision_shape_ref.shape is CapsuleShape3D:
		var cap_shape: CapsuleShape3D = collision_shape_ref.shape as CapsuleShape3D
		default_height = cap_shape.height
		# Ensure crouch_height is smaller than default, keep at least radius*2 + 0.1 and default - 0.7
		if crouch_height >= default_height:
			crouch_height = maxf(cap_shape.radius * 2.0 + 0.1, default_height - 0.7)
		# Also ensure mesh CapsuleMesh is duplicated per-instance so height sync doesn't affect other scenes
		if mesh_ref:
			var mi: MeshInstance3D = null
			if mesh_ref is MeshInstance3D:
				mi = mesh_ref as MeshInstance3D
			else:
				mi = mesh_ref.get_node_or_null("Capsule") as MeshInstance3D
				if mi == null:
					for child: Node in mesh_ref.get_children():
						if child is MeshInstance3D:
							mi = child as MeshInstance3D
							break
			if mi and mi.mesh is CapsuleMesh:
				var cap_mesh: CapsuleMesh = mi.mesh as CapsuleMesh
				if not cap_mesh.resource_local_to_scene:
					mi.mesh = cap_mesh.duplicate(true) as CapsuleMesh
	
	if stair_collision_shape_3d and stair_collision_shape_3d.shape:
...
func _physics_process(delta):
	...
	if ground_check.is_colliding() and is_flying == false:
		...
		if character_node is CharacterBody3D and is_flying == false:
...
func crouch_update(delta: float) -> void:
	if is_flying or character_node == null or collision_shape_ref == null or collision_shape_ref.shape == null or ground_check == null or mesh_ref == null:
		return
	var direct_state: PhysicsDirectSpaceState3D = character_node.get_world_3d().direct_space_state
	var ray_info: PhysicsRayQueryParameters3D = PhysicsRayQueryParameters3D.new()
	ray_info.exclude = [RID(collision_shape_ref)]
	
	ray_info.from = collision_shape_ref.global_transform.origin + Vector3(0, collision_shape_ref.shape.height / 2.0, 0)
	ray_info.to = ray_info.from + Vector3(0, 0.2, 0)
	var collision: Dictionary = direct_state.intersect_ray(ray_info)
	if collision:
		head_bonked = true
	else:
		head_bonked = false
	
	ground_check.position.y = -(collision_shape_ref.shape.height / 2.0) + collision_shape_ref.position.y + 0.2 # Just a small margin

	# Capsule-only mode (OIP Player.tscn has no Skeleton3D): keep visual model stationary and SAME SIZE when crouching.
	# Original AMSG shrinks capsule + lifts skeleton to keep feet planted; for simple CapsuleMesh user prefers no shrinking at all.
	var is_capsule_mode: bool = skeleton_ref == null
	if is_capsule_mode:
		# Keep both collision and visual capsule at default_height (1.7) regardless of stance - no sinking, no shrinking.
		if collision_shape_ref.shape.height != default_height:
			collision_shape_ref.shape.height = default_height
		if mesh_ref:
			var mesh_instance: MeshInstance3D = null
			if mesh_ref is MeshInstance3D:
				mesh_instance = mesh_ref as MeshInstance3D
			else:
				mesh_instance = mesh_ref.get_node_or_null("Capsule") as MeshInstance3D
			...
			if mesh_instance and mesh_instance.mesh is CapsuleMesh:
				var cap_mesh: CapsuleMesh = mesh_instance.mesh as CapsuleMesh
				if cap_mesh.height != default_height:
					cap_mesh.height = default_height
		# No vertical mesh follow - prevents sinking into floor
		return

	if stance == Global.stance.crouching:
		collision_shape_ref.shape.height -= crouch_switch_speed * delta / 2.0
		...
		pass
	mesh_ref.transform.origin.y = clampf(mesh_ref.transform.origin.y, 0.0, 0.5)
	collision_shape_ref.shape.height = clampf(collision_shape_ref.shape.height, crouch_height, default_height)
...
func apply_deacceleration():
	if input_is_moving or is_flying:
		return
	if character_node is CharacterBody3D:
